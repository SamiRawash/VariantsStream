...
def getOmimFields(omim_field):
    if omim_field is not None and omim_field.startswith("Name") :
        ...
        modified=omim_field.replace('\\x',";")
        modified=modified.replace(";3b",";3d")
        modified=modified.replace(";3d",";")[5:]
        try: 
...
class readerVCF_AnnotationFilter(templateCompressReader):
    ...
    def doWithLines(self, line, variantsCounter):
        ...
        if 'ANN' in info and len(info['ANN'])>0:
            ...
            phastCons100way_vertebrate=info['phastCons100way_vertebrate'] if 'phastCons100way_vertebrate' in info else EMPTY_FIELD
            ...
            SIFT_score=info['SIFT_score'] if 'SIFT_score' in info else EMPTY_FIELD
            SIFT4G_score=info['SIFT4G_score'] if 'SIFT4G_score' in info else EMPTY_FIELD
            Polyphen2_HDIV_score=info['Polyphen2_HDIV_score'] if 'Polyphen2_HDIV_score' in info else EMPTY_FIELD
            Polyphen2_HVAR_score=info['Polyphen2_HVAR_score'] if 'Polyphen2_HVAR_score' in info else EMPTY_FIELD
            FATHMM_score=info['FATHMM_score'] if 'FATHMM_score' in info else EMPTY_FIELD
            PROVEAN_score=info['PROVEAN_score'] if 'PROVEAN_score' in info else EMPTY_FIELD
            MetaSVM_score=info['MetaSVM_score'] if 'MetaSVM_score' in info else EMPTY_FIELD
            dbscSNV_ADA_SCORE=info['dbscSNV_ADA_SCORE'] if 'dbscSNV_ADA_SCORE' in info else EMPTY_FIELD
            dbscSNV_RF_SCORE=info['dbscSNV_RF_SCORE'] if 'dbscSNV_RF_SCORE' in info else EMPTY_FIELD
            ...
            for trs in variant_info:
                ...
                GERMLINE_STRING+=f"{omim_inheritance}\t{omim}\t{omim_pathology}\t{flag_lof}\t{lof}\t{flag_nmd}\t{nmd}\t{type_tr}\t{impact}\t{region}\t{annovar_NM}\t{ref}\t{alt}\t{cytoBand}\t"
                ### Predictor: CLINICAL
                ...
                GERMLINE_STRING+=f"{SIFT_score}\t{SIFT4G_score}\t{Polyphen2_HDIV_score}\t{Polyphen2_HVAR_score}\t{dann}\t{FATHMM_score}\t{PROVEAN_score}\t{MetaSVM_score}\t{dbscSNV_ADA_SCORE}\t{dbscSNV_RF_SCORE}\t"
                ...
                GERMLINE_STRING+=f"{GERP_NR}\t{GERP_RS}\t{phyloP100way_vertebrate}\t{phyloP30way_mammalian}\t{phastCons100way_vertebrate}\t{phastCons30way_mammalian}\t{primateai}\t{siphy}\t"
